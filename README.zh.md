...
当前版本里，主 agent 与 task worker 的上下文策略已经分开。主 agent 不再走一次性 `ephemeral` 模式，而是为每个 run 维护一个持久的 Codex planner session，并在后续对话里优先使用 `codex exec resume` 继续同一条 session。同一个 task 的多轮执行现在也会保留自己的 session id。每次真正调用时仍然会启动新的 `codex` 子进程，但第二次及以后会对同一个 task 使用 `codex exec resume`，这样长任务在多次唤醒之间不会把 task 内部上下文完全丢掉。除此之外，宿主还会补上上一次检查后的事件增量、任务 checkpoint 和执行摘要，而不是让模型每轮都从一段松散历史里自己猜上下文。
...
如果主 agent 某一轮思考太久，或者浏览器网络中断后你不想继续等，主界面里的 `Interrupt Planner` 可以显式中断当前 planner turn。这个按钮会终止后端正在运行的 `codex` planner 子进程，而不只是把前端请求取消掉。浏览器流式连接如果意外断开，后端也会自动停止该 planner turn，避免 run 长时间卡在 `Planner is running...`。
...
最近一轮手动验证还包括一个 10 秒周期反复唤醒的场景。在这个场景里，只要 `checkIteration` 能连续达到 `3` 或以上，就视为通过。这说明 orchestrator 不只是撑过一次等待，而是能够按固定周期反复把同一个 task 唤醒回来继续工作。
